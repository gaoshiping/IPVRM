...
OUTPUT_DIR=${OUTPUT_DIR:-${PROJECT_DIR}/step3_distrl/checkpoints/grpo_vr}
...
python -m step3_distrl.main_distrl \
  ...
  "trainer.experiment_name=${EXPERIMENT_NAME:-grpo_vr}" \
  "trainer.nnodes=${NNODES:-1}" \
  "trainer.n_gpus_per_node=${N_GPUS_PER_NODE:-2}" \
  "actor_rollout_ref.actor.distrl_loss_coef=0" \
  "algorithm.reward_gae_coef=0" \
